      ...
      for (const card of cards || []) {
        try {
          ...
          if (searchResults.length === 0) {
            console.log(`[apitcg] No results for: ${card.name}`)
            results.notFound.push(`${dbCategory}:${card.name}`)
            continue
          }
          
          // Card verified as existing
          results.verified++
          ...
          if (matchWithImage && (!card.image_url || card.image_url.includes('placeholder'))) {
            const imageUrl = getBestImage(matchWithImage)

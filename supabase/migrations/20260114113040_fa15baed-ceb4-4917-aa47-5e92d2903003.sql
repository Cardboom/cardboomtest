-- Fix the overly permissive market_items UPDATE policy
-- Only system/admin should update market_items (pricing, images, etc.)
DROP POLICY IF EXISTS "Authenticated users can update market items" ON public.market_items;

-- Create proper policy: only admins can update market items
CREATE POLICY "Admins can update market items"
ON public.market_items
FOR UPDATE
USING (public.has_role(auth.uid(), 'admin'));

-- Fix search_path on validate_auction_duration
...
-- Fix search_path on purchase_pro_pass

...
-- Add column to track who claimed the bounty
ALTER TABLE bounties 
ADD COLUMN IF NOT EXISTS claimed_by_user_id UUID REFERENCES auth.users(id);

-- Create index for lookup
CREATE INDEX IF NOT EXISTS idx_bounties_claimed_by ON bounties(claimed_by_user_id);

-- Update the claim function to store claimer and return their name
...
BEGIN
  ...
  -- Atomically increment total_claimed and set claimed_by_user_id
  UPDATE bounties 
  SET total_claimed = COALESCE(total_claimed, 0) + 1,
      claimed_by_user_id = v_actual_user_id
  ...
  SELECT COALESCE(display_name, 'User') INTO v_user_name
  FROM profiles WHERE id = v_actual_user_id;
  
  RETURN jsonb_build_object(
    'success', true, 
    'gems_awarded', v_gem_amount,

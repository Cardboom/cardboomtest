...
export const WaitlistBanner = ({ onDismiss }: WaitlistBannerProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="relative w-full max-w-sm p-5 sm:p-6 rounded-xl bg-card border border-border shadow-2xl my-auto">
        {/* Close X button */}
        <button
          onClick={onDismiss}
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-4">
          <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-muted/50 text-xs">
            ...
            <span>{t.waitlist.features.pricing}</span>
          </div>
          <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-muted/50 text-xs">
            <Shield className="h-3 w-3 text-primary" />
            <span>{t.waitlist.features.secure}</span>
          </div>
          <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-muted/50 text-xs">
            <Users className="h-3 w-3 text-gold" />
            <span>{t.waitlist.features.verified}</span>
          </div>
        </div>

        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            <Label htmlFor="email" className="text-foreground text-xs">{t.waitlist.form.email}</Label>
            <Input
              id="email"
              type="email"
          ...
          <div>
            <Label className="text-foreground mb-1.5 block text-xs">{t.waitlist.form.interest}</Label>
            <RadioGroup
              value={interest}
            ...
            >
              <div className="flex items-center space-x-1.5">
                <RadioGroupItem value="buyer" id="buyer" className="h-3.5 w-3.5" />
                <Label htmlFor="buyer" className="cursor-pointer text-xs">{t.waitlist.form.buying}</Label>
              </div>
              <div className="flex items-center space-x-1.5">
                <RadioGroupItem value="seller" id="seller" className="h-3.5 w-3.5" />
                <Label htmlFor="seller" className="cursor-pointer text-xs">{t.waitlist.form.selling}</Label>
              </div>
              <div className="flex items-center space-x-1.5">
                <RadioGroupItem value="both" id="both" className="h-3.5 w-3.5" />
                <Label htmlFor="both" className="cursor-pointer text-xs">{t.waitlist.form.both}</Label>
              </div>
            </RadioGroup>
          </div>
